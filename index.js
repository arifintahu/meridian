      ...
      for (const p of actionPositions) {
        ...
        try {
          if (act.action === "CLOSE") {
            ...
            const r = await executeTool("close_position", { position_address: p.position, reason: act.reason || act.rule || "rule exit" });
            ...
            if (!parsed) {
              ...
              await liveMessage?.toolStart("close_position");
              const r = await executeTool("close_position", { position_address: p.position, reason: `instruction: ${parsed.metric} ${parsed.op} ${parsed.value}` });
              const ok = r && r.success !== false && !r.error;
              await liveMessage?.toolFinish("close_position", r, ok);
...
export async function runScreeningCycle({ silent = false } = {}) {
  ...
  try {
    ...
    // Stage entry-time signals for experiment attribution (signal_snapshot).
    if (config.signalStaging?.enabled) {
  ...
  const pnlPollInterval = setInterval(async () => {
    ...
    try {
      ...
      for (const p of result.positions) {
        ...
        const closeRule = getDeterministicCloseRule(p, config.management);
        if (closeRule) {
          const ruleDesc = closeRule.action === "REBALANCE" ? "REBALANCE" : `Rule ${closeRule.rule}: ${closeRule.reason}`;
          const cooldownMs = config.schedule.managementIntervalMin * 60 * 1000;
          const sinceLastTrigger = Date.now() - _pollTriggeredAt;
          if (sinceLastTrigger >= cooldownMs) {
            _pollTriggeredAt = Date.now();
            log("state", `[PnL poll] Deterministic close rule: ${p.pair} — ${ruleDesc} — triggering management`);
            runManagementCycle({ silent: true }).catch((e) => log("cron_error", `Poll-triggered management failed: ${e.message}`));
          } else {
            log("state", `[PnL poll] Deterministic close rule: ${p.pair} — ${ruleDesc} — cooldown (${Math.round((cooldownMs - sinceLastTrigger) / 1000)}s left)`);
          }
          break;
        }
      }
...
function getDeterministicCloseRule(position, managementConfig) {
  ...
  if (
    ...
    const rawVolatility = tracked?.volatility ?? position.volatility;
    const volatilityForRange = Number.isFinite(rawVolatility) && rawVolatility > 0 ? rawVolatility : 3;
...
`);
  ...
  try {
    ...
    console.log(`Top pools (${candidates.length} eligible from ${total_screened} screened):\n`);
  ...
  rl.on("line", async (line) => {
    ...
    if (input === "/candidates") {
      await runBusy(async () => {
        ...
        setLatestCandidates(candidates);
        console.log(`\nTop pools (${candidates.length} eligible from ${total_screened} screened):\n`);
        console.log(formatCandidates(candidates));
        console.log();
      });
      return;
